...
class Line(Problem):
    ...
    def successor (self, state):
        ...
        for i in range(0, l):
            if tempState[i] != 0:
                newState = [el for el in tempState]
                if i < l-1 and newState[i+1] == 0:
                    newState[i+1] = newState[i]
                    newState[i] = 0
                if newState != tempState:
                    successors[f"D1: Disk {tempState[i]}"] = tuple(newState)

                newState = [el for el in tempState]
                if i < l-2 and newState[i+2] == 0 and newState[i+1] != 0:
                    newState[i+2] = newState[i]
                    newState[i] = 0
                if newState != tempState:
                    successors[f"D2: Disk {tempState[i]}"] = tuple(newState)

                newState = [el for el in tempState]
                if i > 0 and newState[i - 1] == 0:
                    newState[i - 1] = newState[i]
                    newState[i] = 0
                if newState != tempState:
                    successors[f"L1: Disk {tempState[i]}"] = tuple(newState)

                newState = [el for el in tempState]
                if i > 1 and newState[i - 2] == 0 and newState[i-1] != 0:
                    newState[i - 2] = newState[i]
                    newState[i] = 0
                if newState != tempState:
                    successors[f"L2: Disk {tempState[i]}"] = tuple(newState)
    ...
    def h(self, node):
        ...
        for i in temp:
            dist += self.goal.index(i) - node.state.index(i)
...
if __name__ == "__main__":
    n = int(input())
    l = int(input())
    start = tuple([i + 1 if i < n else 0 for i in range(0, l)])
    goal = tuple([i if i <= n else 0 for i in start[::-1]])

    line = Line(start, goal)

    result = astar_search(line)
