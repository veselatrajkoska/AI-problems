...
if __name__ == '__main__':
    trainSet = dataset[:math.ceil(0.85 * len(dataset))]
    trainX = [t[:-1] for t in trainSet]
    trainY = [t[-1] for t in trainSet]

    testSet = dataset[math.ceil(0.85 * len(dataset)):]
    testX = [t[:-1] for t in testSet]
    testY = [t[-1] for t in testSet]

    trainX = [[float(trainX[i][j]) for j in range(0, len(trainX[i]))] for i in range(0, len(trainX))]
    testX = [[float(testX[i][j]) for j in range(0, len(testX[i]))] for i in range(0, len(testX))]
