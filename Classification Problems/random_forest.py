...
if __name__ == '__main__':
    ...
    entry = [entry[i] for i in range(0, len(entry)) if i != index]

    dataset = [[dataset[i][j] for j in range(0, len(dataset[i])) if j != index] for i in range(0, len(dataset))]

    trainSet = dataset[:int(0.85 * len(dataset))]
    trainX = [t[:-1] for t in trainSet]
    trainY = [t[-1] for t in trainSet]

    testSet = dataset[int(0.85 * len(dataset)):]
    testX = [t[:-1] for t in testSet]
    testY = [t[-1] for t in testSet]

    classifier = RandomForestClassifier(n_estimators = trees, criterion = criteria)
    classifier.fit(trainX, trainY)

    predictions = classifier.predict(testX)

    correct = 0
    for true, pred in zip(testY, predictions):
        if true == pred:
            correct += 1
    ...
    predictEntry = classifier.predict([entry])
    print(predictEntry[0])
